<template>
  <div class="labels-popover-container">
    ...
    <div
      ...
      class="create-label-backdrop"
      @click="closeCreateLabel"
    >
      <div
        class="create-label-modal"
        @click.stop
      >
...
const availableColors = [
  ...
  '#48DBFB', '#A29BFE', '#FD79A8', '#BADC58', '#F8B500',
  '#FF6348', '#7BED9F', '#70A1FF', '#DFE4EA', '#57606F'
]
...
onMounted(() => {
  ...
  if (props.card.labels) {
    selectedLabels.value = [...props.card.labels]
  }
})

// Watchers
watch(() => showCreateLabel.value, (newValue) => {
  if (newValue) {
    nextTick(() => {
      labelNameInput.value?.focus()
    })
