...
@section('content')
  ...
  <div class="container-fluid flex-grow-1 container-p-y">
    ...
    <div class="modal-dialog modal-xl">
      ...
      <div class="modal-body">
        <small class="text-muted small" id="card-meta-info"></small><!-- mostra nome de quem criou o card e data -->
        <br><br>
      ...
      <div id="modal-{{ $field }}-display" class="editable-field border rounded p-2" data-field="{{ $field }}">
      </div>
      </div>
      @endforeach
  ...
  <script>
    ...
    const kanban = new jKanban({
      ...
      dragBoards: true,
      dragendBoard(el) {
      const columnOrder = Array.from(document.querySelectorAll('.kanban-board')).map((col, index) => ({
      ...
      dropEl(el, target) {
      const columnId = target.closest('.kanban-board').getAttribute('data-id').replace('column-', '');
      ...
      fetch('{{ route('cards.updateOrder') }}', {
        ...
        body: JSON.stringify({ cards: cardIds, column_id: columnId })
      });
      },
      boards: [
    ...
    document.addEventListener('click', function (e) {
      ...
      saveBtn.addEventListener('click', () => {
        ...
        .then(data => {
          ...
          if (data.full_description) {
            icons +=
            `<i class="fa-solid fa-align-left me-2" title="Descrição completa disponível"></i>`;
          }
      ...
      <div class="d-flex align-items-center gap-1 mt-2">${icons}</div>
      </div>`;
          }
        });
      });
      ...
      `;
          ...
          const deleteBtn = li.querySelector('.btn-delete[data-bs-toggle="popover"]');
          if (deleteBtn) {
          ...
          fetch(`/cards/${cardId}`)
            ...
            if (updatedCard.attachments?.length > 0) {
              icons +=
    ...
    if (confirmBtn) {
      ...
      .then(data => {
        ...
        if (cardEl) {
          const iconBlock = cardEl.querySelector('.kanban-card .d-flex.align-items-center.gap-2.mt-2');
    ...
    setTimeout(() => {
      ...
      fetch(`/cards/${cardId}`, {
        ...
        body: JSON.stringify({ status: 1 }) // arquivado
      })
        .then(res => res.json())
        .then(data => {
        if (data.success) {
      ...
      fetch(`/cards/${cardId}`, {
        ...
        body: JSON.stringify({ status: 5 }) // excluído
      })
        .then(res => res.json())
        .then(data => {
        if (data.success) {
    ...
    const isTrigger = e.target.closest('[data-bs-toggle="popover"], .btn-confirm-delete, #btn-archive-options');
  ...
  const dateA = parseInt(a.querySelector('.kanban-card')?.getAttribute('data-created-at')) || 0;
  const dateB = parseInt(b.querySelector('.kanban-card')?.getAttribute('data-created-at')) || 0;
